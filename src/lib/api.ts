...
/**
 * Print photostrip to printer
 * @param photoFile - File object of the photostrip to print
 * @param quantity - Number of copies to print (as string)
...
export async function printPhoto(
  ...
  try {
    ...
    if (!quantityStr || parseInt(quantityStr) < 1) {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(
        errorData.message || `Print failed: ${response.status} ${response.statusText}`
      );
    }
